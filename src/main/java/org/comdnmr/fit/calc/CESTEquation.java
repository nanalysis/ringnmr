...
//                guesses[map[id][7]] = 0.0; //100.0; //R2B
            }
//            for (int i=0; i<map[0].length; i++) {
//                System.out.println(guesses[map[0][i]]);
//            }
...
//                boundaries[1][map[id][1]] = 0.0; //pb UB //guesses[1] * 4;
...
//                boundaries[1][map[id][7]] = guesses[map[id][7]] * 0; //R2B UB
            return boundaries;
        }
     
        @Override
        public int[][] makeMap(int[] stateCount, int[][] states, int[] r2Mask) {
//            int[][] map = {{0,1,2,2,3,3,4,4}};
...
//            }
    ...
    public static String[] getEquationNames() {
        String[] equationNames = {"CESTR1RHOPERTURBATIONNOEX", "CESTR1RHOPERTURBATION", "CESTR1RHOSD", "CESTEXACT0"};//, "CESTR1RHOBALDWINKAY", "CESTR1RHON"};
